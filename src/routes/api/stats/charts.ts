...
import { computeCharts, DEMO_REGISTROS } from "@/lib/stats-service";
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          console.warn("[api/stats/charts] fallback demo:", error);
          const rows = periodo
            ? DEMO_REGISTROS.filter((r) => r.periodo === periodo)
            : DEMO_REGISTROS;
          return Response.json({ ...computeCharts(rows, periodo), demo: true });
        }
      },
    },
  },
