...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div
        className="hidden lg:flex flex-col justify-between p-12 text-sidebar-foreground fade-in"
        style={{ background: "var(--gradient-brand)" }}
      >
        <div className="flex items-center gap-3 slide-right">
          <div className="size-10 rounded-xl bg-primary text-primary-foreground grid place-items-center font-display font-bold shadow-premium-medium">
            C
          </div>
          <span className="font-display text-xl font-semibold tracking-tight text-premium-gold">CONTAM</span>
        </div>
        <div className="space-y-6 max-w-md">
          <h1 className="font-display text-5xl font-semibold leading-tight">
            Sistema Contable Moderno con registros SIRE.
          </h1>
          <p className="text-sidebar-foreground/80 leading-relaxed">
            Gestiona ventas y compras con la estructura extendida SUNAT (35 columnas + campos libres) y
            filtra por periodo, RUC, tipo CDP y más.
          </p>
        </div>
        <p className="text-xs text-sidebar-foreground/60">© 2026 CONTAM — Perú</p>
      </div>

      <div className="flex items-center justify-center p-8 page-enter">
        <form onSubmit={onSubmit} className="w-full max-w-sm space-y-6">
          <div className="space-y-2">
            <div className="size-11 rounded-xl bg-primary text-primary-foreground grid place-items-center mb-3 shadow-premium-medium">
          ...
          <div className="rounded-xl border glass-surface p-4 text-xs text-muted-foreground flex gap-2 shadow-premium-subtle">
