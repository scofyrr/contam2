import { notificationService } from "@/modules/notificaciones/services/notification-service";
import type { NotificacionCorreo, NotificacionTipo } from "@/modules/notificaciones/types/notifications";
import type {
  AlertState,
...
const TASK_TIPOS: NotificacionTipo[] = ["TAREA_ASIGNADA", "TAREA_VENCIDA", "VENCIMIENTO_PROXIMO"];
...
function readDismissed(): Set<string> {
  ...
  try {
    return new Set(JSON.parse(localStorage.getItem(SYSTEM_LOCAL_DISMISSED) ?? "[]") as string[]);
  } catch {
    return new Set();
  }
}
...
function readSnoozes(): Record<string, string> {
  ...
  try {
    return JSON.parse(localStorage.getItem(TASK_SNOOZE_KEY) ?? "{}") as Record<string, string>;
  } catch {
    return {};
  }
}

function writeSnooze(taskId: string, until: string) {
  const all = readSnoozes();
  all[taskId] = until;
  localStorage.setItem(TASK_SNOOZE_KEY, JSON.stringify(all));
}

function mapTipoToSystem(tipo: NotificacionTipo, meta: NotificacionCorreo["metadata"]): SystemNotificationType {
...
function systemIcon(type: SystemNotificationType): string {
  const map: Record<SystemNotificationType, string> = {
...
function systemColor(type: SystemNotificationType): string {
...
function mapNotifToSystem(n: NotificacionCorreo): SystemNotification | null {
...
function computeUrgency(plazo: string | null | undefined, vencida?: boolean): TaskUrgency {
...
function mapPriority(t: TareaPendiente): PendingTask["priority"] {
...
function mapModule(m: string): PendingTaskModule {
...
function badgeColor(urgency: TaskUrgency, priority: PendingTask["priority"]): string {
...
function moduleLink(t: TareaPendiente): string {
...
function mapTareaToPending(t: TareaPendiente, snoozedUntil?: string): PendingTask | null {
  ...
  const recentlyOverdue =
    urgency === "OVERDUE" &&
    t.updated_at != null &&
    Date.now() - new Date(t.updated_at).getTime() < 3600_000;
  ...
  return {
    ...
    metadata: {
      link: moduleLink(t),
      sireRegistroId: t.referencia_id ?? undefined,
    },
...
export function getFABConfig(state: AlertState): FABConfig {
  const { overdueTaskCount, criticalTaskCount, todayTaskCount, fabBadgeCount, fabPulsing, pendingTasks } =
    state;
  ...
  const tooltip =
    parts.length > 0
      ? `${fabBadgeCount} tarea${fabBadgeCount > 1 ? "s" : ""} requieren atención (${parts.join(", ")})`
      : `${state.pendingTaskCount} tareas pendientes`;
...
export class SystemNotificationService {
  async getNotifications(limit = 50): Promise<SystemNotification[]> {
    const raw = await notificationService.obtenerNotificaciones(limit);
    const dismissed = readDismissed();
    const today = todayIso();
    return raw
      .map(mapNotifToSystem)
      .filter((n): n is SystemNotification => n != null)
      .filter((n) => !dismissed.has(n.id))
      .filter((n) => n.expiresAt >= today);
  }
  ...
  async createNotification(input: CreateSystemNotification, userId: string): Promise<string> {
...
export class PendingTaskService {
  async getPendingTasks(ruc?: string): Promise<PendingTask[]> {
  ...
  async getUrgentTaskCount(): Promise<{ overdue: number; today: number; critical: number; total: number }> {
  ...
  async snoozeTask(id: string, until: string): Promise<void> {
...
export class AlertUnifiedService {
  private system = new SystemNotificationService();
  private tasks = new PendingTaskService();

  async getAlertState(ruc?: string): Promise<AlertState> {
  ...
  subscribeToAlerts(callback: (state: AlertState) => void, ruc?: string): () => void {
    let active = true;
    const tick = async () => {
      if (!active) return;
      try {
    ...
    void tick();
    const taskInterval = setInterval(tick, 30_000);
    const sysInterval = setInterval(tick, 60_000);
  ...
  getFABConfig(state: AlertState): FABConfig {
...
export const alertUnifiedService = new AlertUnifiedService();
