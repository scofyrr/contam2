...
function mapTarea(row: Record<string, unknown>): TareaPendiente {
  return {
    ...
    estado: String(row.estado ?? "pendiente") as TareaPendiente["estado"],
    prioridad: String(row.prioridad ?? "media") as TareaPendiente["prioridad"],
    modulo_origen: String(row.modulo_origen ?? "general") as TareaPendiente["modulo_origen"],
...
function applyFiltrosClient(rows: TareaPendiente[], filtros?: TareasFiltros): TareaPendiente[] {
  let out = [...rows];
...
export async function fetchTareas(filtros?: TareasFiltros): Promise<TareaPendiente[]> {
  ...
  if (error) {
    ...
    return applyFiltrosClient((fallback ?? []).map((r) => mapTarea(r as Record<string, unknown>)), filtros);
  }
  return applyFiltrosClient((data ?? []).map((r) => mapTarea(r as Record<string, unknown>)), filtros);
}

export async function fetchTareaById(id: string): Promise<TareaPendiente | null> {
  const { data, error } = await supabase.from(TAREA_VIEW).select("*").eq("id", id).maybeSingle();
  if (error) {
    const { data: fb, error: e2 } = await supabase.from(TAREA_TABLE).select("*").eq("id", id).maybeSingle();
    throwIfSupabaseError(e2, "Error al cargar tarea");
    return fb ? mapTarea(fb as Record<string, unknown>) : null;
  }
  return data ? mapTarea(data as Record<string, unknown>) : null;
}

export async function crearTarea(input: CrearTareaDTO): Promise<TareaPendiente> {
  const payload = {
  ...
  return mapTarea(data as Record<string, unknown>);
}

export async function actualizarTarea(id: string, patch: ActualizarTareaDTO): Promise<void> {
  const body: Record<string, unknown> = { ...patch };
  if (patch.estado === "completada") {
    body.fecha_completada = new Date().toISOString();
  }
...
export async function fetchEstadisticasTareas(ruc?: string | null): Promise<TareasEstadisticas> {
  ...
  if (error || !data?.success) {
    ...
    return {
      ...
      por_modulo: activas.reduce<Record<string, number>>((acc, t) => {
  ...
  return {
    ...
    por_modulo: (d.por_modulo as Record<string, number>) ?? {},
  };
}

export type TareasEstadisticasExtendidas = TareasEstadisticas & {
  efectividad_pct?: number;
  por_mes?: { mes: string; generadas: number; completadas: number }[];
  proyeccion_proximo_mes?: number;
};

export async function fetchEstadisticasTareasMejorada(
  ruc?: string | null,
  periodo?: string | null,
): Promise<TareasEstadisticasExtendidas> {
  ...
  return {
    ...
    por_mes: (d.por_mes as TareasEstadisticasExtendidas["por_mes"]) ?? [],
...
export async function fetchTareasCriticasPendientes(limit = 10): Promise<TareaPendiente[]> {
  const { data, error } = await supabase
    .from(TAREA_VIEW)
    .select("*")
    .in("estado", ["pendiente", "en_progreso"])
    .or("critica.eq.true,plazo_vencimiento.lt." + new Date().toISOString().slice(0, 10))
    .order("plazo_vencimiento", { ascending: true })
    .limit(limit);

  if (error) {
    return fetchTareas({ critica: true, estado: "pendiente", orden: "plazo" });
  }
  return (data ?? []).map((r) => mapTarea(r as Record<string, unknown>));
}
